//! Example: Walrus -> local checkpoint spool -> Sui indexer alt framework -> Parquet (DeepBook).
//!
//! This is a dedicated DeepBook example that only indexes DeepBook events by default.
//! It uses the same light/replicable flow as `alt_checkpoint_parquet`, but with a
//! DeepBook package filter enabled out of the box.
...
const DEFAULT_DEEPBOOK_PACKAGE: &str =
    "0x2c8d603bc51326b8c13cef9dd07031a408a48dddb541963357661df5d3204809";

#[derive(Parser, Debug)]
#[command(name = "deepbook-alt-parquet")]
#[command(about = "Walrus -> Sui indexer alt framework -> Parquet (DeepBook)")]
struct Args {
    ...
    #[arg(long, short, default_value = "./deepbook_events.parquet")]
    output: PathBuf,

    /// DeepBook package ID to filter events (override default)
    #[arg(long, default_value = DEFAULT_DEEPBOOK_PACKAGE)]
    package: String,
...
struct ParquetPipeline {
    package_filter: ObjectID,
    sink: Arc<ParquetSink>,
}

#[async_trait::async_trait]
impl Processor for ParquetPipeline {
    const NAME: &'static str = "deepbook_events";
    ...
    async fn process(&self, checkpoint: &Arc<Checkpoint>) -> Result<Vec<Self::Value>> {
        ...
        if checkpoint_num % 100 == 0 {
            tracing::info!(checkpoint_num, "processing checkpoint");
        }
        ...
        for tx in &checkpoint.transactions {
            ...
            if let Some(tx_events) = &tx.events {
                for (event_index, event) in tx_events.data.iter().enumerate() {
                    if &event.package_id != &self.package_filter {
                        continue;
                    }
                    ...
                    let event_json = deepbook::event_json_from_bcs(&event_type, &event.contents)?;
...
impl Handler for ParquetPipeline {
    ...
    async fn commit<'a>(
        ...
        if !batch.is_empty() {
            tracing::info!(rows = batch.len(), "writing parquet batch");
        }
...
fn duckdb_summary(path: &Path) -> Result<()> {
    ...
    let mut event_sample = conn
        .prepare("SELECT event_json FROM events LIMIT 1")
        .context("failed to prepare event_json sample query")?;
    let mut rows = event_sample.query([])?;
    if let Some(row) = rows.next()? {
        let event_json: String = row.get(0)?;
        println!("DuckDB event_json sample: {}", event_json);
    } else {
        println!("DuckDB event_json sample: <none>");
    }
...
async fn main() -> Result<()> {
    ...
    // Validate a couple of files to fail fast if encoding is incompatible.
    ...
    // Sanity fetch via the framework ingestion client to ensure local store works.
    let metrics = IngestionMetrics::new(None, &prometheus::Registry::new());
    let ingest_client = framework::ingestion::ingestion_client::IngestionClient::new(
        framework::ingestion::ingestion_client::IngestionClientArgs {
